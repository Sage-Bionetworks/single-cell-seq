...
All data should be in a matrix form where the rows represent genes and the columns represent samples. Please see the [scRNA-Seq Instructions]() for more details. 

```{r}
samp.tab<-read.table(synapser::synGet('syn12045100')$path,header=T)%>%select(-c(gene_id,gene_type))%>%rename(Gene=`gene_name`)
